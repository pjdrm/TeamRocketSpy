...
class WeatherBot():
    ...
    def scrape_weather(self):
        ...
        self.weather_forecast = {}
        for h in range(1,5):
            w_hour, w_forecast = self.get_hour_forecast(driver, h)
            self.weather_forecast[w_hour] = w_forecast
        print(self.weather_forecast)
        
    def get_forecast(self):
        current_hour = datetime.now().strftime('%H')
        current_hou_am_pm = int(datetime.strptime(current_hour, "%H").strftime("%I"))
        current_hour = int(current_hour)

        weather_const1 = self.weather_lookup_table[self.weather_forecast[current_hou_am_pm]]
        pogo_weather1 = self.weather_consts[weather_const1]["description"] +\
                        " " + self.weather_consts[weather_const1]["emoji"] 
        weather_const2 = self.weather_lookup_table[self.weather_forecast[current_hou_am_pm+1]]
        ...
        if current_hour+2 not in self.weather_forecast:
